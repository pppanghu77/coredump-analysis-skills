#!/usr/bin/env python3
"""startdde cluster-level automatic fix plans and actions."""
...
def build_fix_plan_for_cluster(cluster: CrashCluster) -> FixPlan:
    # All known startdde crash patterns are SIGABRT (raise abort) with
    # key_frame n/a, or pa_operation_unref related — both without safe
    # application-layer fix points.
    ...
    return FixPlan(
        ...
        root_cause=(
            ...
            "startdde 的可修复崩溃主要为 SIGABRT（raise abort）或 "
            "pa_operation_unref 相关崩溃，应用层没有安全的直接修复点。"
        ),
        fix_description="记录自动分析结果，不修改源码。",
        influence="请结合该根因簇的代表堆栈继续验证，建议向系统层或 PulseAudio 依赖排查。",
    )
